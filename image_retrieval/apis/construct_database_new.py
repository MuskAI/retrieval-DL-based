...
class Database(object):
    ...
    def __init__(self, img_dir='../database', cache_dir='../cache', model_type='oxford',
    ...
    def create_db(self):
        ...
        if os.path.exists(db_path):
            ...
            # 导入模型
            if self.model_type == 'holidays':
                method = FeatExtractor(model_path=self.model_path)
            elif self.model_type == 'oxford':
                method = OxfordFeatExtractor(model_path=self.model_path)
            else:
                assert 'Please input the right model type'
...
if __name__ == '__main__':
    ...
    db = Database(model_type='holidays',model_path='../model_zoo/checkpoint/res18-imagenet-holiday.onnx')
