...
# from model_zoo.cxy_feat import CXYFeat_small
...
class Database(object):
    ...
    def __init__(self, img_dir='../database', cache_dir='../cache', model_type='big',
                 model_path='../model_zoo/checkpoint/res18-imagenet-holiday-partial.onnx'):
        self.img_dir = img_dir
        self.cache_dir = cache_dir
        self.RES_model = model_type
        ...
        self.feat_dim = 512 if model_type is 'big' else 512
    ...
    def create_db(self):
        ...
        if os.path.exists(db_path):
            ...
            # 导入模型
            if self.model_type is 'big':
                method = FeatExtractor(model_path=self.model_path)
            elif self.model_type is 'small':
                pass
                # method = CXYFeat_small(model_path=self.model_path)
            else:
                assert 'Please input the right model type : big or small'
...
if __name__ == '__main__':
    ...
    db = Database(model_type='big',model_path='../model_zoo/checkpoint/res18-imagenet-holiday-partial.onnx')
